...
namespace FastColoredTextBoxNS
{
    public class SyntaxHighlighter : IDisposable
    {
        ...
        protected Regex CSharpAttributeRegex,
            ...
            CALKeywordRegex, CALSystemFunctionsRegex, CALProcedureNameRegex;

        protected Regex CSharpCommentRegex1,
                      CSharpCommentRegex2,
                      CSharpCommentRegex3,
            CALCommentRegex1, CALCommentRegex2, CALCommentRegex3;


        protected Regex CSharpKeywordRegex;
        protected Regex CSharpNumberRegex;
        protected Regex CSharpStringRegex;
        protected Regex CALStringRegex;
        protected Regex CALStringDoubleQuoteRegex;
        ...
        public virtual void FoldingSyntax(Range range)
        {
        ...
        protected void InitCALRegex()
        {
            ...
            CALStringDoubleQuoteRegex = new Regex(@"([^""])*",
            ...
            CALCommentRegex1 = new Regex(@"//.*$", RegexOptions.Multiline | RegexCompiledOption);
            //CSharpCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexCompiledOption);
            //CSharpCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)",
            //                                RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            CALCommentRegex2 = new Regex(@"({.*?})|({.*)", RegexOptions.Singleline | RegexCompiledOption);
            CALCommentRegex3 = new Regex(@"({.*?})|(.*})",
                                            RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            ...
            //CSharpClassNameRegex = new Regex(@"\b(PROCEDURE)\s+(?<range>\w+?)\b",
            //    RegexCompiledOption);
            ...
            //CALKeywordRegex =
            //    new Regex(
            //        @"\b(OnRun|VAR|Record|LOCAL|WITH|DO|BEGIN|IF|THEN|EXIT|END|CASE|repeat|until|AssertError|DownTo|Else|Event|False|For|Local|Of|to|true|while)\b",
...
//";
            ...
            //
            if (CALStringRegex == null)
                InitCALRegex();

            //comment highlighting
            //range.SetStyle(CommentStyle, CALCommentRegex1);
            //range.SetStyle(CommentStyle, CALCommentRegex2);
            //range.SetStyle(CommentStyle, CALCommentRegex3);

            //string highlighting
            //range.SetStyle(StringStyle, CALStringRegex);
            //range.SetStyle(StringDoubleQuoteStyle, CALStringDoubleQuoteRegex);
            ...
            //range.set(new TextStyle(GetBrush("#0000ff"), null, FontStyle.Underline),
            //    new Regex(@"^\s*\bPROCEDURE\b\s"));
            ...
            //range.SetFoldingMarkers(@"BEGIN\b", @"END\b"); //allow to collapse #region blocks
            //range.SetFoldingMarkers(@"CASE\b", @"END\b"); //allow to collapse #region blocks
        ...
        private static void SetPermissionsFolder(Stack<XmlFoldingTag> stack, ref int id, FastColoredTextBox fctb, ref bool isPermissionFolder, Line line, string tagName, int iLine)
        {
            if (line.StartSpacesCount < 16 && isPermissionFolder)
            {
                var tag = stack.Pop();
                if (tag.FoldingStartMarkerCreated)
                {
                ...
                isPermissionFolder = false;
            }
            if (line.Text.StartsWith("    Permissions"))
            {
                var tag = new XmlFoldingTag { Name = tagName, id = id++, startLine = iLine };
                ...
                isPermissionFolder = true;
                stack.Push(tag);
            }
        }
        private static void SetVARFolder(Stack<XmlFoldingTag> stack, ref int id, FastColoredTextBox fctb, ref bool isVarFolder, Line line, string tagName, int iLine)
        {
            if (Regex.Match(line.Text, @"\bBEGIN\b").Success && isVarFolder)
            {
                var tag = stack.Pop();
                if (tag.FoldingStartMarkerCreated)
                {
            ...
            if (line.Text.EndsWith("=VAR") || line.Text.StartsWith("    VAR"))
            {
                var tag = new XmlFoldingTag { Name = tagName, id = id++, startLine = iLine };
