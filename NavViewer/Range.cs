...
namespace FastColoredTextBoxNS
{
    ...
    public class Range : IEnumerable<Place>
    {
        ...
        public bool Contains(Range rr)
        {
            return Contains(rr.Start) && Contains(rr.End);
        }
        public bool Icr()
        {
            foreach (var rr in tb.Poc)
            {
                if (rr.Contains(this)) return true;
            }
            return false;
        }
        ...
        public bool CheckHaveStyle(Style style)
        {
            //search code for style
            StyleIndex styleIndex = ToStyleIndex(tb.GetOrSetStyleLayerIndex(style));
            ...
            for (int y = fromLine; y <= toLine; y++)
            {
                ...
                for (int x = fromX; x <= toX; x++)
                {
                    ...
                    Char c = tb[y][x];
                    if (!c.style.HasFlag(styleIndex)) return false;
                    //if (c.style != styleIndex) return false;
                    //c.style |= styleIndex;
                    //tb[y][x] = c;
                }
            }
        ...
        public IEnumerable<Range> GetRangesByLineReversed(string regexPattern, RegexOptions options)
        {
            //var regex = new Regex(regexPattern, options);
            foreach (var r in GetRangesByLinesReversed(regexPattern, options))
                yield return r;
        }
